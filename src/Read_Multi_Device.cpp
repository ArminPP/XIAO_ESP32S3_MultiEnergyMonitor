...
#include <esp_task_wdt.h> // Watchdog timer (attention, there are differences between Arduino V2 and V3 implementation!)
...
bool EmOnCMSconnectionStatus      = false; // to check if connection to EmonCMS is established and show it in the webserver
// sometimes, the ESP32 has problems with the connection to EmonCMS, so it is good to know if the connection is established

char RESET_REASON[50] = {'\0'}; // reset reason
...
code borrowed from:
https://github.com/espressif/arduino-esp32/blob/496b8411773243e1ad88a68652d6982ba2366d6b/libraries/ESP32/examples/ResetReason/ResetReason2/ResetReason2.ino#L22C1-L39C2
*/
...
// **********************************************************************
{
    switch (r)
    {
        ...
        case ESP_RST_POWERON: return "Power on or RST pin toggled";               // Power on or RST pin toggled
        ...
        case ESP_RST_INT_WDT: return "Interrupt watchdog (software or hardware)"; // Interrupt watchdog (software or hardware)
        case ESP_RST_TASK_WDT: return "Task watchdog";                            // Task watchdog
        case ESP_RST_WDT: return "Other watchdog";                                // Other watchdog
        case ESP_RST_DEEPSLEEP: return "deep sleep";                              // Reset after exiting deep sleep mode
        case ESP_RST_BROWNOUT: return "Brownout reset (software or hardware)";    // Brownout reset (software or hardware)
...
/*
  get the reset reason of the ESP and store it in a global variable for the webserver
*/
void getResetReason()
// **********************************************************************
// **********************************************************************
{
    ...
    snprintf(RESET_REASON, sizeof(RESET_REASON), "%s", resetReasonName(reason));
}
...
This is a little bit more economic, than writing the same code for everey needed delay
*/
...
/*
  ...
  checks every 5 seconds if WiFi is connected, if not, it tries to reconnect
...
// **********************************************************************
{
    ...
    */
    ...
    WiFi.setSortMethod(WIFI_CONNECT_AP_BY_SIGNAL); // for mesh/repeater (e.g. FritzBox) use the strongest AP!
    WiFi.setScanMethod(WIFI_ALL_CHANNEL_SCAN);     // for mesh/repeater (e.g. FritzBox) use the strongest AP!
    ...
    // WiFi timeout
    int16_t timeout = 20; // 20 * 0.5 sec to connect to a WiFi, otherwise create an AP or show timeout message ...
            ...
            {                           // if there's bytes to read from the client,
                ...
                { // if the byte is a newline character
                    ...
                    if (currentLine.length() == 0)
                    {
                        ...
                        client.println("<head>");
                        client.println("    <title>::: Powermeter Multi V3 :::</title>");
                        ...
                        // web page body
                        client.println("<body>");
                        client.printf("<h1>Powermeter Multi V3.%s Beta</h1>", BUILD_NUMBER);
                        client.println("<hr>");
...
// **********************************************************************
{
    ...
    Serial.println();
    Serial.println();
    Serial.printf(" Current Solar PZEM004T address:       0x%02x\r\n", Solar.readAddress());
    Serial.printf(" Current Consumption PZEM004T address: 0x%02x\r\n", Consumption.readAddress());
    Serial.printf(" Current Grid PZEM004T address:        0x%02x\r\n", Grid.readAddress());
    Serial.println();
    /*
    ...
    */
    ...
    esp_task_wdt_init(WDT_TIMEOUT_SECONDS, true); // Init Watchdog with xx seconds timeout and panicmode
    esp_task_wdt_add(NULL);                       // No special task needed
    Serial.printf("WDT is running: %s\r\n", esp_task_wdt_status(NULL) ? "true" : "false");
...
// **********************************************************************
{
    ...
    if (WiFi.status() == WL_CONNECTED)
    {
        ...
        WIFIclient.setTimeout(5);                                      // it is SECONDS! not milliseconds
